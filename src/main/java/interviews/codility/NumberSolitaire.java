...
public class NumberSolitaire {
    ...
    public static void main(String args[]) {
        ...
        int result = ns.solution(new int[] {1, -2, 0, 9, -1, -2});
        System.out.println(result);

        int result2 = ns.solution(new int[] {-1, -5, 2, -5, -5, -1});
        System.out.println(result2);

        int result3 = ns.solution(new int[] {5});
    ...
    private int move(int[] board, int square) {

        // are we on final square?
        ...
        // attempt to find highest value moves from this point
        // for all possible dice roles
        for (int k = 1; k <= 6; k++) {

            int newSquare = square + k;

            if(isValidMove(board, newSquare)) {

                // attempt to find highest winning path
                int sumMoves = move(board, newSquare) + board[square];

                if (bestMove == null || sumMoves > bestMove) {
                    bestMove = sumMoves;
                }
            }
        }
    ...
    private boolean isValidMove(int[] board, int square) {
